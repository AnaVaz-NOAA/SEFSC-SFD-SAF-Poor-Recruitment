...
CHLSeason <- read.csv("Chl_EOF_Seasonal.csv")
CHLModis  <- read.csv("Chl_EOF_Seasonal_Modis.csv")
#
yearsPlot <- 1993:2021
...
for (iSpp in 1:10) {
  for (iEOF in 2:itot) {
    ...
    # Create a data frame with your data
      ...
      png(gsub(" ","",paste("../images/CorrelationCHLSpSeason/",spNamePlot[iSpp],NamePlotSeason[iEOF-1],".png")),res = 300, width = 2000, height = 2000)
...
png(gsub(" ","",paste("../images/Correlation_CHL_SpSeason_Species_heatmap.png")),res = 300, width = 2000, height = 2000)
...
melted_cor_matrix <- melt(plot_matrix)
png(gsub(" ","",paste("../images/Correlation_CHL_SpSeason_Species_heatmap05.png")),res = 300, width = 2000, height = 2000)
ggplot(melted_cor_matrix, aes(x = Var1, y = Var2, fill = value)) +
  geom_tile(color = "white", size = 0.5) +  # Add white grid lines
  scale_fill_gradient2(low = "#3B9AB2", mid = "white", high = "#F21A00", na.value = "white", midpoint = 0) +
  theme_minimal() +
...
for (iSpp in 1:10) {
  for (iEOF in 2:itot) {
    ...
    # Create a grid of the two plots
    png(gsub(" ","",paste("../images/CorrelationCHLModis/",spNamePlot[iSpp],NamePlotSeason[iEOF-1],".png")),res = 300, width = 2000, height = 2000)
    grid.arrange(plot1, plot2, ncol = 1)
    dev.off()
  } # end iEOF, lwd 
}  # end iInd
...
melted_cor_matrix <- melt(plot_matrix)
png(gsub(" ","",paste("../images/Correlation_CHL_Modis_Season_Species_heatmap.png")),res = 300, width = 2000, height = 2000)
ggplot(melted_cor_matrix, aes(x = Var1, y = Var2, fill = value)) +
  geom_tile(color = "white", size = 0.5) +  # Add white grid lines
  scale_fill_gradient2(low = "#3B9AB2", mid = "white", high = "red", na.value = "white", midpoint = 0) +
  theme_minimal() +
