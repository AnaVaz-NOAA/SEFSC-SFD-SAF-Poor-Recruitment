...
for (iVar in seq_along(NamePlot)) {
  ...
  for (iMode in seq_along(NameModes)) {
    ...
    fig_name <- paste0(pathfigs, myname[iVar],'_linear_mode',iMode,'_CNAPS.png')
    cat(paste0('![](', fig_name, ')\n'),sep="")
    fig_name <- paste0(pathfigs, myname[iVar],'_linear_temporal_mode',iMode,'_CNAPS.png')
    cat(paste0('![](', fig_name, ')\n\n'))
  }
  cat("::: \n\n")
}
...
for (iVar in seq_along(NamePlot)) {
  ...
  for (iSeason in seq_along(NameSeason)) {
    for (iMode in seq_along(NameModes)) {
      ...
      cat(paste0('![](', fig_name, ')\n'))
      fig_name <- paste0(pathfigs, myname[iVar],'_linear_',NameSeason[iSeason],'_temporal_mode',iMode,'_CNAPS.png')
      cat(paste0('![](', fig_name, ')\n\n'))
    }  
  }
  cat("::: \n\n")
...
for (iVar in seq_along(NamePlot)) {
  for (iSeason in seq_along(NameSeason)) {
    ...
    fig_name <- paste0(pathfigs, myname[iVar],'_',NameSeason[iSeason],'_anomaly.png')
    cat(paste0('![](', fig_name, ')\n\n'),sep="")
  }
}
...
for (iVar in seq_along(NamePlot)) {
  ...
  for (iSeason in seq_along(NameSeasonSp)) {
    for (iMode in seq_along(NameModes)) {
      ...
      fig_name <- paste0(pathfigs, myname[iVar],'_linear_Spawning_',NameSeasonSp[iSeason],'_mode',iMode,'_CNAPS.png')
      cat(paste0('![](', fig_name, ')\n'))
      fig_name <- paste0(pathfigs, myname[iVar],'_linear_Spawning_',NameSeasonSp[iSeason],'_temporal_mode',iMode,'_CNAPS.png')
      cat(paste0('![](', fig_name, ')\n\n'))
    }  
  }  
  cat("::: \n\n")
