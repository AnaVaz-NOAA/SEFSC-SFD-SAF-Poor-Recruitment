...
for ivar = 1:1
    ...
    for iseason = 1:2
      ...
      for i=1:points;
        varDetrend(:,i) = detrend(seasonal_avg_sp(:,i,iseason),'linear');
      end
      eval(['save varDetrend_',myname,'_',sName,' varDetrend']);

      lim = max([max(abs(mean(varDetrend,2))) max(abs(mean(varDetrendmean,2)))]);
      hFig = figure('pos',[100 100 1000 500]);
      set(hFig,'Visible','off');
      title('Detrend linear');
      anomaly(unique(yy), mean(varDetrend,2));
      axis([min(yy) max(yy) -lim lim]);
      eval(['print -dpng ',myname,'_',sName,'_anomaly.png']);
      close all;

      % calculate EOFs

      varSeason = varDetrend;
      mynameP = [myname, '_linear'];
    ...
    for iseason = 1:4
      ...
      varDetrendmean = zeros(Ntime,points);
      for i=1:points;
        varDetrend(:,i) = detrend(seasonal_avg(:,i,iseason),'linear');
      end
    ...
    varDetrend = zeros(Ntime,points);
    for i=1:points;
      varDetrend(:,i) = detrend(varM(:,i),'linear');
    end
