...
for (iSpp in 2:11) {
    ...
    for (cpt in cpaux@cpts) {
      breaks_matrix <- rbind(breaks_matrix, c(spNamePlot[iSpp-1], 'CPT', yraux[cpt]))
    }
    ...
    par(mfrow = c(2,1), mar=c(0.5, 0.5, 0.5, 0.5), oma=c(0, 0, 0, 0), mai=c(0.7, 0.7, 0.7, 0.7))
    
    plot(years, rep(0,length(years)), ylim=c(-limit, limit), type="n", ylab="", xlab="", main = c("Change Point ",spNamePlot[iSpp-1]))
...
for (iEOF in 2:length(EOFs)) {
  ...
  for (cpt in cpaux@cpts) {
    breaks_matrix <- rbind(breaks_matrix, c(NamePlotModes[iEOF-1], 'CPT', yraux[cpt]))
  }
  ...
  plot(years, rep(0,length(years)), ylim=c(-limit, limit), type="n", 
       ylab="", xlab="", main = c("Change Point ",NamePlotModes[iEOF-1]))
  axis(1, at = seq(min(years), max(years), by = 1), labels = NA, tck = -0.02)
  lines(yraux, aux,  lty=1, col=4, lwd=2)
  abline(v = yraux[cpaux@cpts], col = "lightgray", lwd = 5)
